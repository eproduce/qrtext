...
- fix: 修复 AppImage 缺失 libstdc++.so.6 和 GSettings schemas
- docs: update changelog for 0.4.0 [skip ci]
...
- docs: update changelog for 0.4.0 [skip ci]
- fix: 修复 AppImage 在麒麟上的 GLIBCXX_3.4.29 not found 错误
- docs: update changelog for 0.4.0 [skip ci]
- revert: 回退原生截图，使用系统截图工具
- feat: 原生截图 — Rust xcap 全屏捕获 + 选区裁剪 (v0.4.0)
- docs: update changelog for 0.3.8 [skip ci]
- fix(ci): AppImage 解释器终极修复 — AppRun 直接调用 ld-linux
- docs: update changelog for 0.3.8 [skip ci]
- fix: 退出确认弹窗 UI 优化 — 居中 + 图标
- fix: 合并重复 :style 属性消除 Duplicate attribute 报错
- fix: 截图编辑器显示+保存双高清
- fix: 截图编辑器保存/复制使用原图分辨率而非显示尺寸
- docs: update changelog for 0.3.8 [skip ci]
- fix: 退出确认弹窗修复 — 自定义 Vue 弹窗替代 confirm()
- fix: 移除顶层 await 避免 Vue Suspense 警告
- fix: 修正窗口关闭逻辑 — 编辑态只关编辑器 + 主界面退出确认
- fix: 编辑器关闭行为与文字输入位置修复
- docs: update changelog for 0.3.8 [skip ci]
- fix(ci): AppImage 解释器路径修复 — 相对路径替代绝对路径
- docs: update changelog for 0.3.8 [skip ci]
